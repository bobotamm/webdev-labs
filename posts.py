...
class PostListEndpoint(Resource):
    ...
    def get(self):
        ...
        valid_id = get_authorized_user_ids(self.current_user)
        post = Post.query.filter(Post.id.in_(valid_id))
        ...
        if limit:
            ...
            if limit > 50 or limit < 1:
                return Response(json.dumps({'message': 'Limit must be integers between 1 and 50'}), mimetype="application/json", status=400)

            posts = post.limit(limit).all()

        # data = Post.query.limit(post_limit).all()
...
def initialize_routes(api):
    ...
    # Detail (GET) singlie, PATCH, and DELETEE
    api.add_resource(
        PostDetailEndpoint, 
