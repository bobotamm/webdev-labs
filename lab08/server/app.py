...
# way to log in, disconnect, and chat
...
async def respond_to_message(websocket, message):
    ...
    # goal: respond to different messages differently 

    type = data.get('type')

    # log in to web socket server
    if type == 'login':
        print('login')
    ...
    # when any of clients send a message, relay message to anyone who is logged in
    elif type == 'chat':
        print('chat')
        for socket in logged_in_users:
        ...
        # send message to all users that a client has left 
        for socket in logged_in_users:
...
async def broadcast_messages(websocket, path):
    ...
    # handle when client disconnect 
        ...
        # remove user from queue(server memeory) 
